<script lang="ts">
  ...
  let { form, currPopUp = $bindable(), divisions = $bindable() }: { form: ActionData, currPopUp: String, divisions:DivisionDTO[] } = $props();

  let linkableServices = $state([])
        ...
        <div class="w-3/5  bg-white p-6 flex flex-col gap-3 ">
          ...
          <label>
            ...
            <select name="serviceType" class="input-box rounded w-full">
              {#each linkableServices as {}}
                <option>Services</option>
              {/each}
            </select>

            <button type="button" class="mt-2 font-semibold text-purple-500 rounded-lg hover:text-purple-200" data-sveltekit-reload>
            + Create New Service
            </button>
          </label>
