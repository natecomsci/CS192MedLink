...
export class ERServiceDAO {
  async create(facilityID: string, employeeID: string, data: CreateERServiceDTO): Promise<void> {
    try {
      await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
        ...
        await updateLogDAO.createUpdateLog(
          { type: "Emergency Room", action: Action.CREATE },
          facilityID,
          employeeID,
          tx
        );
  ...
  async update(serviceID: string, facilityID: string, employeeID: string, data: ERServiceDTO): Promise<void> {
    try {
      await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
        ...
        await updateLogDAO.createUpdateLog(
          { type: "Emergency Room", action: Action.UPDATE },
          facilityID,
          employeeID,
          tx
        );
